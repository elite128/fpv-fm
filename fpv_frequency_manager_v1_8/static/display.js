...
function render() {
  ...
  $("stats").innerHTML = `
    ...
    <span>${state.stats.used_channels} Belegte Kanäle</span>
    <span>${state.stats.conflicts} Konflikte</span>
  `;
  
  const list = $("pilotList");
  if (!state.pilots.length) {
    list.innerHTML = `<div class="display-empty">Noch keine Piloten auf der Frequenztafel eingetragen.</div>`;
    return;
  }
  
  list.innerHTML = state.pilots.map(p => {
    const ch = p.channel || "-";
    const freq = p.frequency ? `${p.frequency} MHz` : "Kein Kanal";
    
    let conflicts = "✓ Frequenz sicher";
