...
def reload_config_in_memory(cfg: dict) -> None:
    ...
    PUBLIC_HOSTNAME = os.environ.get("PUBLIC_HOSTNAME", CONFIG.get("server", {}).get("public_hostname", "rotorhazard.localdomain"))
    PUBLIC_PORT = int(os.environ.get("PORT", CONFIG.get("server", {}).get("port", 8000)))
    USE_HTTPS = os.environ.get("USE_HTTPS", str(CONFIG.get("server", {}).get("use_https", False))).lower() in ("true", "1", "yes")
...
def check_ip_limit(name: str, client_ip: Optional[str]) -> None:
    ...
    for p in pilots.values():
        if p.ip_address == client_ip and p.name != name:
            raise HTTPException(
                status_code=403,
                detail=f"Registrierung verweigert: Nur 1 Pilot pro Gerät/IP erlaubt (bereits belegt durch '{p.name}')."
            )
...
def set_pilot_channel(name: str, channel: Optional[str], created_by: str = "self", force: bool = False) -> Optional[dict]:
    ...
    if ch not in CHANNELS:
        raise HTTPException(status_code=400, detail=f"Unbekannter Kanal: {ch}")
    if ch in locked_channels:
        raise HTTPException(status_code=409, detail=f"{ch} ist gesperrt")
    owner = next((p.name for p in pilots.values() if p.channel == ch and p.name != name), None)
    if owner:
        raise HTTPException(status_code=409, detail=f"{ch} ist bereits von {owner} belegt")
    freq = CHANNELS[ch]
...
async def admin_update_pilot(req: AdminUpdatePilotRequest):
    ...
    if old not in pilots:
        raise HTTPException(status_code=404, detail="Pilot nicht gefunden")
...
async def admin_toggle_lock(req: AdminChannelRequest):
    ...
    if ch not in CHANNELS:
        raise HTTPException(status_code=400, detail="Unbekannter Kanal")
    if ch in locked_channels:
...
async def admin_save_config(req: SaveConfigRequest):
    ...
    try:
        ...
        if not isinstance(parsed, dict):
            raise ValueError("YAML muss auf oberster Ebene ein Dictionary sein.")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"YAML-Syntaxfehler: {e}")
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Fehler beim Speichern der Konfiguration: {e}")
