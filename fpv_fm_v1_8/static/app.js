...
async function join() {
  ...
  try {
    ...
    toast(`Profile saved as ${currentUser}`);
    render();
  } catch (e) {
    toast(e.message);
  }
...
async function selectChannel(channel, force = false) {
  if (!currentUser) return toast("Please enter your pilot name first!");
  try {
    ...
    toast(channel ? `${channel} successfully claimed` : "Frequency released");
  } catch (e) {
    toast(e.message);
  }
}
...
function showConfirm(data) {
  $("confirmText").textContent = `${data.channel} (${data.frequency} MHz) is very close to claimed frequencies:`;
  $("confirmConflicts").innerHTML = data.conflicts.map(c =>
    `<div class="conflicts" style="color: var(--red); font-weight: 700; margin: 0.2rem 0;">
      ⚠ ${escapeHtml(c.channel || "?")} (${escapeHtml(c.pilot)}) · ${c.frequency} MHz (spacing: ${c.diff} MHz)
     </div>`
  ).join("");
...
function render() {
  ...
  $("stats").innerHTML = `
    ...
    <span>${state.stats.used_channels} claimed</span>
    <span>${state.stats.conflicts} conflicts</span>
    <span>${state.stats.locked} locked</span>
  `;
  renderPilots();
  renderChannelGroups();
...
function renderPilots() {
  ...
  if (!state.pilots.length) {
    list.innerHTML = `<p class="muted" style="padding: 1rem 0;">No pilots registered yet.</p>`;
    return;
  }
  list.innerHTML = state.pilots.map(p => {
    const ch = p.channel || "No channel";
    const freq = p.frequency ? `${p.frequency} MHz` : "Not assigned";
    const source = p.created_by === "admin" ? "Admin" : p.created_by === "import" ? "Import" : "Self";
    ...
    if (p.conflicts && p.conflicts.length > 0) {
      ...
      conflictHtml = `<span class="conflict-badge" style="color: var(--green);">✓ Safe</span>`;
    }
...
function channelTile(c) {
  ...
  else if (mine) status = "your channel";
  else if (used) status = `occupied: ${c.owner}`;
  else if (c.severity === "yellow") status = "near";
  else if (c.severity === "red") status = "critically near";
...
function renderRecommendations() {
  ...
  if (!state.recommendations.length) {
    el.innerHTML = `<p class="muted" style="grid-column: 1/-1; padding: 0.5rem 0;">No conflict-free channels available.</p>`;
    return;
  }
...
function renderAdmin() {
  ...
  $("adminPilots").innerHTML = state.pilots.length ? state.pilots.map(p => `
    <div class="admin-row admin-pilot-row">
      ...
      <button data-save="${escapeHtml(p.name)}">Save</button>
      <button data-remove="${escapeHtml(p.name)}" class="danger">Remove</button>
    </div>`).join("") : `<p class="muted">No pilots registered.</p>`;
  ...
  $("adminChannels").innerHTML = state.all_channel_groups.map(group => `
    <div class="admin-group"><h4>${escapeHtml(group.name)}</h4><div class="admin-channel-grid">
      ${group.channels.map(c => `<button class="channel-tile ${c.locked ? "locked" : ""}" data-lock="${escapeHtml(c.name)}">
        ...
        <span class="tile-freq">${c.frequency} MHz</span>
        <span class="tile-status">${c.locked ? "unlock" : "lock"}</span>
      </button>`).join("")}
    </div></div>`).join("");
...
window.addEventListener("online", () => toast("Connected to network"));
window.addEventListener("offline", () => toast("You are offline. Showing local offline copy."));
...
$("copyLinkBtn").addEventListener("click", async () => {
  try { await navigator.clipboard.writeText(state.server_url); toast("Event link copied to clipboard!"); }
  catch { toast(state.server_url); }
});

$("copyDisplayLinkBtn").addEventListener("click", async () => {
  try { await navigator.clipboard.writeText(state.display_url); toast("Display link copied to clipboard!"); }
...
$("bulkImportBtn").addEventListener("click", async () => {
  try {
    ...
    toast(data.errors?.length ? `Import errors: ${data.errors.join(" | ")}` : `${data.added.length} pilots successfully imported`);
  } catch (e) { toast(e.message); }
});

$("resetBtn").addEventListener("click", async () => {
  if (!confirm("Race Control Warning: Really delete all pilots, frequencies and locks?")) return;
  try { await api("/api/admin/reset", { password: adminPassword() }); toast("Event reset completely"); }
  catch (e) { toast(e.message); }
});
...
async function tryLoadConfig() {
  ...
  if (!pwd) {
    ...
    configArea.placeholder = "Please enter password to load configuration.";
  ...
  try {
    ...
    configArea.placeholder = "Please enter the correct password to load configuration.";
    configArea.disabled = true;
    $("saveConfigBtn").disabled = true;
    configLoaded = false;
  }
